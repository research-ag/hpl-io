// An HPL agent is a http agent with ability to schedule call and obtain request id before sending it

// Current file is fully copy-pasted HttpAgent code with few changes since we cannot easily extend it anymore because
// of extensive use of private properties and functions
...
import { SubnetStatus, request } from '@dfinity/agent/lib/cjs/canisterStatus';
 ...
 * There is a pipeline to apply transformations to the request before sending
 ...
 allowing extensions.
 */
export class HplAgent implements Agent {
  ...
  public static createSync(options: HttpAgentOptions = {}): HplAgent {
  ...
  public static async create(
    ...
    const agent = HplAgent.createSync(options);
  ...
  public static async from(agent: Pick<HplAgent, 'config'> | V1HttpAgentInterface): Promise<HplAgent> {
    try {
      if ('config' in agent) {
        return await HplAgent.create(agent.config);
      }
      return await HplAgent.create({
        fetch: agent._fetch,
