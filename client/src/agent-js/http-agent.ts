...
import { AnonymousIdentity, Identity } from '@dfinity/agent/lib/cjs/auth';
import * as cbor from '@dfinity/agent/lib/cjs/cbor';
import { hashOfMap, RequestId, requestIdOf } from '@dfinity/agent/lib/cjs/request_id';
import { bufEquals, bufFromBufLike, concat, fromHex, toHex } from '@dfinity/agent/lib/cjs/utils/buffer';
import {
  Agent,
  ApiQueryResponse,
  HttpDetailsResponse,
  ...
  SubmitResponse,
  v3ResponseBody,
} from '@dfinity/agent/lib/cjs/agent/api';
...
import {
  ...
  AgentQueryError,
  AgentReadStateError,
} from '@dfinity/agent/lib/cjs/agent/http/errors';
import { request, SubnetStatus } from '@dfinity/agent/lib/cjs/canisterStatus';
import { CertificateVerificationError, HashTree, lookup_path, LookupStatus } from '@dfinity/agent/lib/cjs/certificate';
import { ed25519 } from '@noble/curves/ed25519';
import { ExpirableMap } from '@dfinity/agent/lib/cjs/utils/expirableMap';
import { Ed25519PublicKey } from '@dfinity/agent/lib/cjs/public_key';
import { decodeTime } from '@dfinity/agent/lib/cjs/utils/leb';
import { ObservableLog } from '@dfinity/agent/lib/cjs/observable';
import { BackoffStrategy, BackoffStrategyFactory, ExponentialBackoff } from '@dfinity/agent/lib/cjs/polling/backoff';
...
export class HttpAgent implements Agent {
  ...
  public static async from(agent: Pick<HttpAgent, 'config'> | V1HttpAgentInterface): Promise<HttpAgent> {
    try {
      if ('config' in agent) {
  ...
  public addTransform(type: 'update' | 'query', fn: HttpAgentRequestTransformFn, priority = fn.priority || 0): void {
    if (type === 'update') {
  ...
  private async call0(
    ...
    if (Math.abs(this.#timeDiffMsecs) > 1_000 * 30) {
      ingress_expiry = new Expiry(this.#maxIngressExpiryInMinutes * MINUTE_TO_MSECS + this.#timeDiffMsecs);
    }
    ...
    const nonce: Nonce | undefined = transformedRequest.body.nonce ? toNonce(transformedRequest.body.nonce) : undefined;
  ...
  private async call1(
    arg: {
      requestId: RequestId;
      ecid: Principal;
      transformedRequest: HttpAgentSubmitRequest;
      body: ArrayBuffer;
      callSync: boolean;
      backoff: BackoffStrategy;
      submit: CallRequest;
    },
    v3UnsupportedCallBack: () => Promise<SubmitResponse>,
  ): Promise<SubmitResponse> {
    const { requestId, ecid, transformedRequest, body, callSync, backoff, submit } = arg;
    try {
  ...
  public async call(
    ...
    const arg = await this.call0(canisterId, options, identity);
    return this.call1(arg, () => {
      return this.call(
        canisterId,
        {
          ...options,
  ...
  public async prepareCall(
    ...
    identity?: Identity | Promise<Identity>,
  ): Promise<{ requestId: RequestId; commit: () => Promise<SubmitResponse> }> {
    const arg = await this.call0(canisterId, options, identity);
    return {
      requestId: arg.requestId,
      commit: () =>
        this.call1(arg, async () => {
          throw new Error('V3 unsupported! try to set options.callSync to false');
        }),
    };
  }
   ...
   */
  #verifyQueryResponse = (queryResponse: ApiQueryResponse, subnetStatus: SubnetStatus | void): ApiQueryResponse => {
  ...
  public async syncTime(canisterId?: Principal): Promise<void> {
    await this.#rootKeyGuard();
    const CanisterStatus = await import('@dfinity/agent/lib/cjs/canisterStatus');
  ...
  public async status(): Promise<JsonObject> {
    ...
    const response = await this.#requestAndRetry({
      backoff,
      request: () => this.#fetch('' + new URL(`/api/v2/status`, this.host), { headers, ...this.#fetchOptions }),
      tries: 0,
    });
